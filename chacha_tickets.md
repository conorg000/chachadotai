...
Set up TypeScript, build tooling (e.g. tsup or tsc), linting, basic scripts.

Define and export shared interfaces in packages/core/src/types.ts:
...
Export placeholder classes from @safetylayer/core with empty implementations:

export class SessionEngine {
constructor(opts?: { maxMessages?: number }) {}
ingestMessage(msg: Message): SessionState { throw new Error('NYI'); }
getSession(sessionId: string): SessionState | undefined { return undefined; }
listSessions(): SessionState[] { return []; }
...
async analyze(record: CoTRecord): Promise<CoTRecord> { return record; }
}

Dependencies: None.
Unblocks: Tickets 2, 3, 4, 5, 6, 7.
...
Append message, prune old ones beyond maxMessages.

Compute a simple placeholder risk score (e.g., messages.length / 20 clamped to [0,1]).

Append RiskSnapshot to timeline.
...
Parallel with: Ticket 3, Ticket 4.
...
Scope:
Implement internal LLMSessionDetector in packages/core:
...
Call LLMSessionDetector.run on relevant messages (e.g. on user messages or every turn).

Update SessionState.riskScore and patterns using returned risk_score and patterns.

Update timeline and trigger onRiskThreshold/onPattern callbacks based on new values.

Provide configuration to turn LLM detection on/off (for local dev).

Dependencies: Ticket 2 (it replaces its stub scoring), Ticket 1.
Parallel with: Ticket 4, Ticket 5 (API can initially work with stub and then plug this in).
...
Add one or two canonical labels:

cot_deception, goal_drift, policy_evasion.

Ensure CoTMonitor has no state beyond the current call; it just analyzes a single assistant turn’s CoT.

Optionally export a helper type or function for common usage:

export type CoTMonitorResult = CoTRecord;

Dependencies: Ticket 1.
Parallel with: Ticket 2, Ticket 3, Ticket 5 (API will call this).
...
Scope:
Scaffold an Express or Fastify app in /apps/demo-api.

Wire SessionEngine and CoTMonitor from @safetylayer/core.

Implement endpoints:

POST /chat

Payload: { sessionId: string, userMessage: string }.

Flow:

Build Message for user, call sessionEngine.ingestMessage.

Call model (e.g. OpenAI) to get both assistant answer and a CoT-style block (either real or simulated).

Build Message for assistant, call sessionEngine.ingestMessage.

If CoT present, call cotMonitor.analyze(...).

Response: { assistant: Message, session: SessionState, cot?: CoTRecord }.
...
Dependencies: Ticket 2 (SessionEngine), Ticket 4 (CoTMonitor), Ticket 1.
Parallel with: Ticket 6, Ticket 7.
...
Scope:
Scaffold React app (Next.js or Vite) in /apps/dashboard.
...
Table with: sessionId, riskScore (with badge color), patterns, last message preview.
...
Dependencies: Ticket 5 (API shape), Ticket 1.
Parallel with: Ticket 3, Ticket 4, Ticket 7.
...
Short description (session-aware behavioral security + CoT monitoring).

Installation (monorepo local or hypothetical npm install @safetylayer/core).
...
Dependencies: Ticket 2, Ticket 4, Ticket 1.
Parallel with: Ticket 3, Ticket 5, Ticket 6.
...
Expect risk to climb 0.1 → 0.9 and patterns like gradual_escalation to appear.

“CoT deception”:

Prompt the model into a response whose CoT suggests hiding info or ignoring rules.

Expect cot_deception label, non-trivial CoTAnalysis.riskScore.
...
Document demo flow in README or a DEMO.md:
...
Dependencies: Ticket 5 (API), Ticket 3 (LLM-based detector), Ticket 4 (CoTMonitor).
Parallel with: Ticket 6, Ticket 7.

Ticket 9 (optional) – Minimal tests + logging
Goal: Add just enough tests and logging to look credible.
Scope:
Add a couple of unit tests in @safetylayer/core:

SessionEngine adds/prunes messages and updates timeline.

SessionEngine triggers onRiskThreshold callbacks when risk crosses threshold (can mock detector).
...
Parallel with: Any remaining tickets.
