...
description: 'Error classes, retry logic, and error handling patterns'
---

## Error Classes

The SafetyLayer SDK provides three custom error classes for different failure scenarios:
...
<AccordionGroup>
  <Accordion title="Always Handle Errors" icon="shield-check">
    Never let SafetyLayer errors crash your application. Use try-catch blocks and log errors appropriately.
  </Accordion>

  <Accordion title="Fail Open for Non-Critical Paths" icon="door-open">
    If SafetyLayer is down, decide whether to block all traffic (fail closed) or continue with degraded safety (fail open).
  </Accordion>

  <Accordion title="Log Error Details" icon="file-lines">
    Include error codes, status codes, and request IDs in logs for debugging:
    ```typescript
    console.error('SafetyLayer error:', {
      code: error.code,
  ...
  <Accordion title="Monitor Error Rates" icon="chart-line">
    ...
    - High 401 errors → Check API keys
    - High 429 errors → Implement backoff or increase limits
    - High 5xx errors → Backend issues
  </Accordion>
