...
Central shared types defined once and reused everywhere. Implemented in Ticket 1 and used by all others.
...
2. Call `LLMSessionDetector.run(session.messages)`:
...
3. Fire callbacks if thresholds/patterns matched
...
**Flow:**
1. Build `Message` for user and call `sessionEngine.ingestMessage`
2. Call LLM for:
   - Assistant answer (surface content)
   - CoT (either real or simulated via prompt patterns)
3. Build assistant `Message` and call `sessionEngine.ingestMessage`
4. If CoT exists, call `cotMonitor.analyze` to get `CoTRecord`
...
- Then **parallel**:
  - Ticket 2 (SessionEngine base)
  - Ticket 4 (CoTMonitor interface)
